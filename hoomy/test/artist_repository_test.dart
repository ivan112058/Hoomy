...
void main() {
  ...
  test('歌手详情取到该歌手的专辑', () async {
    ...
    final artist = await ArtistRepository(fakeClient(transport)).getArtist('ar1');

    expect(artist.name, 'Adele');
    expect(artist.albums.map((a) => a.name), ['25', '21']);
  });
  ...
  test('歌手全部歌曲：逐张专辑取曲目后打平，按 id 去重', () async {
    ...
    final songs =
        await ArtistRepository(fakeClient(transport)).getArtistSongs('ar1');

    expect(songs.map((s) => s.id), ['s1', 's2', 's3']);
    expect(songs.map((s) => s.title), ['Hello', 'Send My Love', 'Rolling in the Deep']);
  });
